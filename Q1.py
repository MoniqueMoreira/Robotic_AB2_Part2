...
def plots(qs,erro,t):
    ...
    plt.plot(erro,t, label = "Erro X Tempo")
    plt.legend()
    plt.show()
...
def resolved_rate_control_2r(L1 = 1,L2=1):
    ...
    for i in range(len(Ts)):
        ...
        J = np.array([[-(L1 + L2*np.cos(theta2))*np.sin(theta1),-L2*np.cos(theta1)*np.sin(theta2)],
                  [np.cos(theta1)*(L1 + L2*np.sin(theta2)),L2*np.cos(theta2)*np.sin(theta1)]])
        #print(J)
        #print(jsingu(J))
        ...
        if abs(determinant) < 1e-6:
            lambda_value = 0.02  # Ajuste este valor conforme necessário
